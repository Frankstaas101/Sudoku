...
public class BacktrackingSolver {
	 ...
	 */
	public boolean solve(Cell nextCell)
	{
		...
		if(nextCell == null) {
			...
			//Cell selectedCell = nextCell);
			puzzle.findPossibleValues(nextCell);
			
			for(int i = 0; i < nextCell.possibleValues.size(); i++) {
				nextCell.value = nextCell.possibleValues.get(i);
				//Set the cell to the value above in the puzzle.
				puzzle.cells[nextCell.x][nextCell.y].value = nextCell.value;
...
>>>>>>> refs/remotes/origin/master
	...
	//					sortUnassignedCells();
				...
				//reset the cell
				puzzle.cells[nextCell.x][nextCell.y].value = 0;
			}
